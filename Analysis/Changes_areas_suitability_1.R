...
#thresholds
thresh=cbind(crops, c(0.50,0.50,0.50,0.50,0.50)*100)#Change this for each crop to necces threshold
...
res[[1]] = matrix(ncol = 5, nrow = 19, dimnames=list(models, crops))# making a dataframes that we will fill, 8 rops 19 gcms
res[[2]] = matrix(ncol = 5, nrow = 19, dimnames=list(models, crops))
res[[3]] = matrix(ncol = 5, nrow = 19, dimnames=list(models, crops))
...
region_code=c("Ecuador")
crop_names=c("Cocoa","Coffee", "Maize","Banana","Cassava")
...
axis(1)
axis(2, labels = crop_names, at=seq(1,5,1), las=1) #at=seq(1,8,1), ##las= 1 Tells R to put the labels horizantal for the y axis.. Y axis (axis 2) in R always
abline(v=c(seq(20,80,20)), col="gray", lty=2) ##Tells R to giveit horizantal dashed line at from -30 - 20 at intervals of 10
title(main = paste("Suitability Change (%) until 2050s", sub = "under A2"))
legend("topright",legend=paste(region_code[1]),box.col="black",cex=0.8) #This prints for each boxplot the corresponding region
...
axis(1)
axis(2, labels = crop_names, at=seq(1,5,1), las=1) #at=seq(1,8,1), ##las= 1 Tells R to put the labels horizantal for the y axis.. Y axis (axis 2) in R always
abline(v=c(seq(-30,30,10)), col="gray", lty=2) ##Tells R to giveit horizantal dashed line at from -30 - 20 at intervals of 10
title(main = paste("Suitability Change (%) until 2050s", sub = "under A2"))
legend("topright",legend=paste(region_code[1]),box.col="black",cex=0.8) #This prints for each boxplot the corresponding region
...
title(main = paste("Suitability Change (%) until 2050s", sub = "under A2"))
legend(pos_legend[2],legend=region_code,box.col="black",cex=0.8) #This prints for each boxplot the corresponding region
